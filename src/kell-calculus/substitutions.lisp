...
(defgeneric substitute (process mapping &optional ignored-vars)
  ...
  (:method (process mapping &optional ignored-vars)
    ...
    (values process nil))
  (:method ((name variable) mapping &optional ignored-vars)
    (if (find (label name) ignored-vars)
        (values name nil)
  ...
  (:method ((process named-concretion) mapping &optional ignored-vars)
    (values (make-instance 'named-concretion
                           ...
                           :continuation (substitute (continuation process)
                                                     mapping
                                                     ignored-vars)
  ...
  (:method ((process pattern-abstraction) mapping &optional ignored-vars)
    (multiple-value-bind (new-patterns sub-patterns-p)
      ...
      (let* ((pattern (pattern process))
             (ignored-vars (append (mapcar #'label
                                           (bound-names *current-pattern-language*
                                                        pattern))
                                   (mapcar #'label
                                           (bound-variables *current-pattern-language*
                                                            pattern))
                                   ignored-vars)))
